...
object Chapter5 extends App{
 val colm = new Counter()
  ...
  class Counter{
    private var value = Int.MaxValue-2//var chagess
    def increment(){if(value + 1 < Int.MaxValue){
      value += 1
    }}
    def current() = value
  }
  class BankAccount{
    ...
    def balance = bal
    def withdraw(withdrawAmmount:Int){bal -= withdrawAmmount}
  }
    //read only
    ...
    class Time {
      ...
      def this(hrs:Int, min:Int) {//constrators
        this()
        this.h = hrs*60-1
        this.m = min
      }
